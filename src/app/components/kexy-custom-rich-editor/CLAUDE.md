...
- **Drag-to-reposition**: media blocks are `draggable="true"` (set on insert + in `hydrateEditorBlocks`) and moved via native HTML5 DnD delegated on the canvas (`dragstart`/`dragover`/`drop`/`dragend`). `dropLocationFromPoint` picks the drop CONTAINER — a `td`/`th` cell under the cursor (so blocks can be dropped INTO a table), else the canvas — then the nearest direct child of that container decides before/after (midpoint), or appends when there's no child. Indicators: `.drop-before`/`.drop-after` insertion line on the ref child, `.drop-into` outline on an empty container/cell. Dragstart on the `.resize-handle` is suppressed so resizing isn't hijacked. NOTE: reposition is a direct DOM move, so it is NOT on the browser undo stack (unlike insert). Export (`transformMediaBlocks`) and `getRawHtml` both query `.media-block` globally, so blocks nested in cells still export/round-trip.

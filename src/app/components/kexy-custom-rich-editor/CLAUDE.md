...
- **Merge-tag chips are inserted with the Range API** (`insertInlineNodeAtCaret`: `range.insertNode` at the caret + a trailing space), NOT `execCommand('insertHTML')`. insertHTML promotes the chip to a NEW LINE when the caret is at the END of a paragraph; the Range insert keeps it inline in the current block. Trade-off: chip insertion is not on the native undo stack (matches the approved build). Same helper handles body + subject insertion.

...
- Order: Source(`</>`) │ lists (bullet/numbered/checklist) │ undo/redo │ image/link │ Paragraph select │ font family select │ font size select │ B I U S │ alignment dropdown │ (spacer) │ `⋮` overflow.
- The `⋮` **overflow menu** holds secondary tools: text/highlight color, table, remove-link, video, merge-tag `#` picker. (Font family/size used to live here but now sit inline in the main strip next to the Paragraph select.)
...
- **Video poster from a real frame**: `onVideoFile` calls `EditorUtilsService.captureVideoPoster(file)` — loads the file into an off-DOM `<video>` (object URL → same-origin, untainted canvas), seeks ~0.2s in, draws the frame to a canvas, returns a JPEG data URL + real dimensions (block width stays 420, height from the true ratio). The poster is uploaded via the same `uploadImage` callback (hosted URL in exports), falling back to the inline frame, and finally to the generic `buildVideoPlaceholderSvg` play-button thumbnail if decoding fails. The captured poster flows through the existing `data-poster` path, so the badge and export keep working unchanged.
- **Custom video thumbnail**: when a video block is selected, the media inspector shows an "Upload custom thumbnail…" button (video-only) → `EditorCanvasComponent.replaceSelectedVideoPoster(file)`. It uploads the chosen image via the shared `uploadImage` uploader (now wired by the host onto BOTH the toolbar and the canvas), falling back to an inline data URL, then updates the visible `.video-thumb img` src and `data-poster` so design/preview/export all use the new thumbnail.
- **Play overlay** is a CSS `.video-thumb::after` pseudo-element (NOT in the block markup) so it shows on every video thumb — real captured frames AND already-saved videos — and never needs hydration. It's built to stay visible on ANY thumbnail color: a semi-transparent dark disc (contrast on light frames) + a white ring and white triangle (contrast on dark frames) + a drop shadow; sized with `clamp()` + `%` so it scales with the thumb and stays circular. It's editor/design-view only — the email export uses the poster image (linked to the video URL) with no overlay (email clients can't reliably composite one).

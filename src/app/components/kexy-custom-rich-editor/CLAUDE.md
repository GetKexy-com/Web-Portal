...
- Flat **inline-SVG icons**, grouped with `.tool-divider` separators. `.toolbar-scroll` is **`flex-wrap: nowrap; overflow: hidden`** — the toolbar is **always a single row** at every width; it never wraps and never squishes.
- **Priority-plus overflow.** The main strip is built from collapsible **clusters** (`GROUPS` in `editor-toolbar.component.ts`), each rendered as an `<ng-template>` (`#tplSource`, `#tplLists`, `#tplAlign`, `#tplHistory`, `#tplInsert`, `#tplParagraph`, `#tplFontFamily`, `#tplFontSize`, `#tplTextFormat`). A `ResizeObserver` on `.toolbar-scroll` measures each cluster's cached natural width and — keeping highest-`priority` first — pushes the clusters that don't fit into the `⋮` menu (`recomputeOverflow()`, `visibleGroups()`/`overflowGroups()`, `overflowIds` signal). Collapse order (first to go) is by ascending priority: fontSize → fontFamily → paragraph → align → insert → lists → history → textFormat → source (so **B/I/U/S and Source survive longest**). The SAME `TemplateRef` renders in either the strip or the menu via `ngTemplateOutlet` (no markup duplication; handlers/signals keep working). Widths are cached per cluster while it's in the strip; all clusters start visible so the first pass measures every one.
- Display order: Source(`</>`) │ lists (bullet/numbered/checklist) │ alignment dropdown │ undo/redo │ image/link/remove-link/table │ Paragraph select / font family select / font size select │ B I U S │ `⋮` overflow (right-pinned via `margin-left:auto`).
- The `⋮` **overflow menu** is **always present** and holds the **collapsed clusters** (when the strip is narrow) PLUS the tools that permanently live there: **text/highlight color**, **video**, and the **merge-tag `#` picker**. In the menu, clusters' in-strip `.tool-divider`s are hidden (`.overflow-menu .tb-group .tool-divider { display:none }`).

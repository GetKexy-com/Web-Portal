...
export class EditBrandComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    let website = localStorage.getItem(constants.PROSPECTING_WEBSITE);
    if (website !== '') {
      website = JSON.parse(website)[0];
    } else {
      website = "";
    }

    this.userData.supplier_phone = this.pageUiService.formatUSPhoneNumbers(`${this.userData.supplier_phone}`);
  ...
  async primaryFormSubmitted() {
    ...
    this.httpService.patch(`company/${this.userData.supplier_id}`, data).subscribe((response) => {
      if (response.success) {
        ...
        localStorage.setItem(constants.PROSPECTING_WEBSITE, data.websites ?? '');
